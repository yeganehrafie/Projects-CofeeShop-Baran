...
const Salad_ProductsHome = ({ truncateText, handleshowDetailsProducts }) => {
  const dispatch = useDispatch();
  const cartItems = useSelector((state) => state.cart.carts);
  const handleAddToCart = (product) => {
    dispatch(addToCart(product));
  };
  const products_salad = React.useMemo(
    () => [
      {
        id: 10,
        title: "زیتون پرورده ",
...
export default Salad_ProductsHome;
